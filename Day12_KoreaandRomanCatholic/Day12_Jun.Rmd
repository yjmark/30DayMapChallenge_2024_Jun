---
title: "Day 12 Challenge - Korea and Roman Catholic"
...
bgMap <- bgMap %>%
  ...
  mutate(
    sovereignt_old = case_when(
      ...
      sovereignt %in% c("China", "Taiwan") ~ "Ming-Qing",
      TRUE ~ sovereignt  # 나머지 국가들은 그대로 유지
    )
  ) %>%
...
ggplot() +
  ...
  xlim(110, 150) +  # Set x-axis limits for the map (longitude range)
  ylim(30, 45) +  # Set y-axis limits for the map (latitude range)
  geom_sf(data=bgMap, fill=NA, color="#fff")
``` 
